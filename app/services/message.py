...
# In-memory cache for session history to prevent repetitive DB reads
# Max 1000 sessions, 2 hours TTL
HISTORY_CACHE = TTLCache(maxsize=1000, ttl=7200)
...
async def get_message_by_hash(
    db: AsyncSession, session_id: str, message_hash: str
) -> Optional[Message]:
    """Check if a message with this hash already exists for the session."""
    query = select(Message).where(
        Message.session_id == session_id, Message.message_hash == message_hash
    )
...
async def add_message(
    db: AsyncSession, session_id: str, role: RoleEnum, content: str
) -> Message:
    ...
    if session_id in HISTORY_CACHE:
        HISTORY_CACHE[session_id].append({"role": role.value, "content": content})
...
async def get_session_history(db: AsyncSession, session_id: str) -> List[dict]:
    ...
    if session_id in HISTORY_CACHE:
        return list(HISTORY_CACHE[session_id])
