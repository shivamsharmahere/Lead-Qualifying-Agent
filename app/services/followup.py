...
async def process_follow_ups(db: AsyncSession):
    ...
    inactivity_threshold = now - timedelta(minutes=settings.FOLLOW_UP_INACTIVITY_MINUTES)
    ...
    for lead in eligible_leads:
        ...
        if recent_msgs.scalars().first():
            logger.info(f"Skipping follow-up for {lead.session_id}, user replied recently.")
            continue
        ...
        db.add(lead)
        logger.info(f"Sent follow-up {lead.follow_up_count} to session {lead.session_id}")
